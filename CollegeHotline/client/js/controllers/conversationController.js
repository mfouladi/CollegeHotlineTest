...
app.controller('conversationController', ['$scope', '$resource', function ($scope, $resource) {
	...
	function updatePage() {
		...
		InactiveConversations.query({
			active: false
		}, function (results) {
			$scope.inactiveConversations = results;
		});

		ActiveConversations.query({
			active: true
		}, function (results) {
	...
	$scope.activateConversation = function (index, activatePhoneNumber) {
		Activate.get({
			...
			setTimeout(function () {
				$scope.openConversation($scope.activeConversations.length - 1, activatePhoneNumber);
			}, 5);
		});
	};
	...
	$scope.deactivateConversation = function (index, deactivatePhoneNumber) {
		Deactivate.get({
			phoneNumber: deactivatePhoneNumber
		}, function (result) {
